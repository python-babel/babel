...
def get_sorted_authors_list():
    ...
    recent_authors = Popen(['git', 'log', '--format=- %aN <%aE>', tag_range],
                    stdout=PIPE).communicate()[0].rstrip().split('\n')

    all_authors = Popen(['git', 'log', '--format=- %aN <%aE>'],
                    stdout=PIPE).communicate()[0].rstrip().split('\n')

    # Count the number of commits from the most recent tag
    counts = Counter(recent_authors)

    # Include all authors
    counts.update(set(all_authors))
