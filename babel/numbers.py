...
def spell_number(number, ordinal=False, locale=LC_NUMERIC, **kwargs):
    ...
    Might be a bad idea to have locale specific options in kwargs
    so use it as a last resort if the data is not available in the
    locale or in the specific speller class as a class attribute
    (e.g. which scale to use if there are multiple possibilities)
    """
